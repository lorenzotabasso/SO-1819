...
int main (int argc, char * argv[]) {
    ...
    read_conf(CONF_PATH);
    printf("(PID: %d) Configurazione caricata.\n", getpid());
     ...
     * simulation.
     */
	 if (semctl(id_children_semaphore, 0, GETVAL)== 0) {
		for(int j = 0; j < POP_SIZE; j++){
			if(relase_resource(id_children_semaphore, 0)==-1) // 0 -> the first semaphonre in the set
				perror("Errore nella prima release di parent");
		}
	}


    /* Parent initialize the matrix for the marks in the shared memory. */
    set_shared_data();
...
void parent_handle_signal(){
    ...
    sleep(2); // needed only for correct displaying the marks on the command line
